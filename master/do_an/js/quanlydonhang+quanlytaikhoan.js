...
function loadOrderList(orderElm, orderList) {
    ...
    for (let i = 0; i < orderList.length; ++i) {
            ...
            if (ans == 1) {
                try {
                    const response = await fetch(
                        "http://localhost:8080/hoadon/accept/" +
                            orderList[i].mahoadon,
                        {
                            method: "PUT",
                            headers: {
        ...
        deny.addEventListener("click", async () => {
            ...
            let ans = confirm(
                "Bạn chắc chắn muốn hủy,thao tác không thể hoàn tác!?"
            );
            if (ans == 1) {
                try {
                    const response = await fetch(
                        "http://localhost:8080/hoadon/deny/" +
                            orderList[i].mahoadon,
                        {
                            method: "PUT",
                            headers: {
...
function conditionSearch(condition) {
    ...
    if (condition.localeCompare("Date") == 0) {
        ...
        Array.from(orderList).forEach((element) => {
            ...
            console.log(d);
            if (d >= fromDateInput && d <= toDateInput)
